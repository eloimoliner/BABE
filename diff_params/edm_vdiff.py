...
"""
This class includes all the functions related to the the EDM diffusion parameterization, following ( Karras et al., "Elucidating...", 2022).
I include a wrapper for v-diffusion as implemented in k-diffusion https://github.com/crowsonkb/k-diffusion/blob/master/k_diffusion/external.py
"""
...
class EDM():
    ...
    def __init__(self, args):
        ...
        self.apply_vdiff_wrapper=self.args.diff_params.apply_vdiff_wrapper
        if self.apply_vdiff_wrapper:
            #v-diffusion assumes sigma_data=1
            self.sigma_data=1
    ...
    def denoiser(self, xn , net, sigma):
        ...
        if self.apply_vdiff_wrapper:
            cnoise=self.sigma_to_t(sigma)
            cnoise=cnoise.squeeze(-1)
            cnoise=cnoise.expand(xn.shape[0])
            #important detail!, there is a negative sign in cout
            cout=-cout


            #xn_stereo=xn.unsqueeze(1).expand(-1,2,-1) #fake stere0
            out=net(cin*xn, cnoise)
            #out=out[:,0,:] #get first channel only
            return cskip * xn +cout*out #this will crash because of broadcasting problems, debug later!

            #alpha, beta =self.sigma_to_alpha_beta(sigma)
            #v_t = net(alpha*xn, cnoise)
            #pred_t = xn*alpha**2  - v_t * beta
            #return pred_t
    ...
    #methofd from the k-diffusion repo:
    def sigma_to_t(self, sigma):
        return sigma.atan()*(2/math.pi)

    def t_to_sigma(self, t):
        return (t * math.pi / 2).tan()


    def sigma_to_alpha_beta(self, n):
        t=self.sigma_to_t(n)
