...
class EDM():
    ...
    def denoiser_cfg(self, xNn , net, sigma, xSn=None, xTn=None, CLAP=None, cfg_args=None):
        """
        This method does the whole denoising step, which implies applying the model and the preconditioning
        Args:
            x (Tensor): shape: (B,T) Intermediate noisy latent to denoise
        ...
        if cfg_args.mode=="alltogether":
            ...
            #batch two copies of XNn
            xNn_in = torch.cat([xNn] * 4)
            cnoise_in=torch.cat([cnoise]*4)
          
            #same xSn, but zero the second one
            xSn_in=torch.cat([xSn, xSn*0, xSn*0, xSn*0],dim=0)
            xTn_in=torch.cat([xTn*0, xTn,xTn*0, xTn*0],dim=0)
            CLAP_in=torch.cat([CLAP*0, CLAP*0, CLAP, CLAP*0],dim=0)
            ...
            out_cond_S, out_cond_T, out_cond_CLAP, out_uncond=net(cin*xNn_in, cnoise_in, cinS*xSn_in, cinT*xTn_in, CLAP_in).chunk(4, dim=0)
            #return cskip * xNn +cout*((1+cfg_CLAP)*out_cond_CLAP -cfg*out_uncond)  #this will crash because of broadcasting problems, debug later!
            return cskip * xNn +cout*(out_uncond+cfg_CLAP*(out_cond_CLAP-out_uncond)+cfg_S*(out_cond_S-out_uncond)+cfg_T*(out_cond_T-out_uncond))  #this will crash because of broadcasting problems, debug later!
        else:
            raise NotImplementedError


    def denoiser_only_cfg(self, x, net, sigma, xSn, xTn, CLAP=None, cfg_args=None, x_uncond=None):
        if len(sigma.shape)==1:
            sigma=sigma.unsqueeze(-1)
        ...
        x_in=torch.cat([x]*2)
        xSn_in=torch.cat([xSn, xSn*0])
        xTn_in=torch.cat([xTn*0, xSn])

        out_cond_S, out_cond_T=net(cin*x_in, cnoise, cin*xSn_in, cin*xTn_in, CLAP*0).chunk(2, dim=0)

        #shit, we need to denormalize that
        out_uncond=(x_uncond-cskip*x)/cout

        return cskip * x +cout*(out_uncond+cfg_S*(out_cond_S-out_uncond)+cfg_T*(out_cond_T-out_uncond))  #this will crash because of broadcasting problems, debug later!


    def denoiser(self, xNn , net, sigma, xSn=None, xTn=None, CLAP=None):
        """
        This method does the whole denoising step, which implies applying the model and the preconditioning
        Args:
            x (Tensor): shape: (B,T) Intermediate noisy latent to denoise
        ...
        if self.args.exp.CLAP.use_CLAP:
            return cskip * xNn +cout*net(cin*xNn, cnoise, cin*xSn, cin*xTn, CLAP)  #this will crash because of broadcasting problems, debug later!
        else:
            return cskip * xNn +cout*net(cin*xNn, cnoise, cin*xSn, cin*xTn)  #this will crash because of broadcasting problems, debug later!
    ...
    def prepare_train_preconditioning(self, x, xS, xT, sigma):
        ...
        try:
            ...
            print("Warning: noise_ST not defined, using same_as_x")
            #if that variable is not defined, just use the same noise for all (same_as_x)
            noiseS=self.sample_prior(xS.shape,sigma.unsqueeze(1))
            noiseT=self.sample_prior(xT.shape,sigma.unsqueeze(1))
            cinS=cin
    ...
    def loss_fn(self, net, xN, xS, xT, CLAP_latent=None ):
        ...
        #if self.args.diff_params.spectrogram.use_conditioning:
            #input a copy of xN to the spectrogram conditioner, without gradient
        #    SpecN=net.apply_STFT_conditioner(xN.detach().clone())
        #    SpecN=self.CFG_dropout(SpecN)
        #print("xNspec",xNspec)
        #print(xS.shape, xT.shape, CLAP_latent.shape)
        #try:
        #if self.args.diff_params.spectrogram.use_conditioning:
        #    #print("conditioning on spectrogram")
        #    
        #    #print("memory usage before forward", torch.cuda.memory_allocated() / 1e9)
        #    estimate=net(input,cnoise, xS, xT, CLAP_latent, SpecN)
            #print("memory usage after forward", torch.cuda.memory_allocated() / 1e9)
        #else:
        if self.args.exp.CLAP.use_CLAP:
                estimate=net(input,cnoise, xS, xT, CLAP_latent*0)
        else:
                print("no CLAP")
                estimate=net(input,cnoise, xS, xT)
