...
from utils.decSTN_pytorch import STN_transient_enhancer
...
class EDM():
    ...
    def __init__(self, args):
        ...
        if self.args.diff_params.transient_enhancer.gain>=0:
            device='cuda' if torch.cuda.is_available() else 'cpu'
            self.transient_enhancer=STN_transient_enhancer(self.args.exp.sample_rate,self.args.diff_params.transient_enhancer.nwin, G1=self.args.diff_params.transient_enhancer.G1, G2=self.args.diff_params.transient_enhancer.G2, gain=self.args.diff_params.transient_enhancer.gain, device=device)
    ...
    def loss_fn(self, net, x):
        ...
        if self.args.diff_params.transient_enhancer.gain>=0:
            error=self.transient_enhancer.forward(x,error)
