...
class Unet_CQT_noDC(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        """
        super(Unet_CQT_noDC, self).__init__()
        self.args=args
        ...
        self.CQTransform=CQT_nsgt(self.args.network.cqt.num_octs, self.args.network.cqt.bins_per_oct, mode="matrix_pow2",window=win,fs=self.args.exp.sample_rate, audio_len=self.args.exp.audio_len, dtype=torch.float32, device=self.device)
        ...
        self.init_block=ResnetBlock(Nin, self.Ns[0], self.use_norm,num_dils=1, bias=False, kernel_size=(1,1), emb_dim=self.emb_dim, init=init, init_zero=init_zero)
        self.out_block=ResnetBlock(self.Ns[-1], 2, use_norm=self.use_norm,num_dils= 1,bias=False, kernel_size=(1,1), proj_place="after", emb_dim=self.emb_dim, init=init, init_zero=init_zero),
        ...
        for i in range(self.depth):
            ...
            self.downs.append(
                               nn.ModuleList([
                                        ...
                                        ResnetBlock(dim_in, dim_out, self.use_norm,num_dils=self.num_dils[i], bias=False , attention_dict=attn_dict, emb_dim=self.emb_dim, init=init, init_zero=init_zero, Fdim=self.f_dim)
                                        ]))
        ...
        for i in range(self.depth-1,-1,-1):
            ...
            self.ups.append(nn.ModuleList(
                                        ...
                                        ResnetBlock(dim_in, dim_out, use_norm=self.use_norm,num_dils= self.num_dils[i],attention_dict=attn_dict, bias=False, emb_dim=self.emb_dim, init=init, init_zero=init_zero, Fdim=self.f_dim),
                                        ]))
    ...
    def forward(self, inputs, sigma):
        ...
        for i,modules in enumerate(self.downs):
            ...
            if i==0:
                ...
                pyr=self.downsamplerT(pyr) #updating the auxiliary path
        ...
        Out=Xout.permute(0,2,3,1).contiguous() #call contiguous() here?
        Out=torch.view_as_complex(Out)

        #save output
        Out=Out.unsqueeze(1)
        pred_time=self.CQTransform.bwd(Out)
        pred_time=pred_time.squeeze(1)
        print(pred_time.shape, inputs.shape)
