...
class BiasFreeGroupNorm2(nn.Module):
    def __init__(self, num_channels, num_groups, eps=1e-5):
        ...
        # Create a bias parameter for each group

        #self.bias = nn.Parameter(torch.zeros(1, self.channels_per_group, 1, 1))
    ...
    def forward(self, x):
        ...
        # Reshape the input tensor to (N * num_groups, channels_per_group, H, W)
        x = x.view(N, self.num_groups, self.channels_per_group, H, W)
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        x = x.view(N * self.channels_per_group, self.num_groups, H, W)

        # Calculate the mean and std of the input tensor
        #mean = x.mean(dim=2, keepdim=True).mean(dim=3, keepdim=True)
        std = x.std(dim=2, keepdim=True).std(dim=3, keepdim=True)

        # Normalize the input tensor
        x = (x) / (std + self.eps)

        # Reshape the normalized tensor and apply the scale and bias
        x = x.view(N, self.channels_per_group, self.num_groups, H, W)
        x = x.permute(0, 2, 1, 3, 4).contiguous()
        x = x.view(N, C, H, W)
        x = x * self.weight 
...
class Unet_CQT_oct(nn.Module):
    ...
    def __init__(self, args, device):
        ...
        self.middle.append(nn.ModuleList([
                        ResnetBlock(self.Ns[self.depth], self.Ns[self.depth], self.use_norm, num_dils=self.num_dils[-1], bias=False)
                        ]))
