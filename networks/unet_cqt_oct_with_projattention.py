...
class TimeAttentionBlock(nn.Module):
    def __init__(self, Nin,attention_dict, init, init_zero, Fdim) -> None:
        ...
        self.proj_out=Conv2d(attention_dict.num_heads, Nin, (1,1), bias=False, **init_zero)
        ...
        self.norm = BiasFreeGroupNorm(attention_dict.num_heads,1)
...
class ResnetBlock(nn.Module):
    ...
    def forward(self, input_x, sigma):
        ...
        if self.attention_dict is not None:
            ...
            #x=self.proj_attn_in(x)
            #print("preattnblock",x.shape)           
            #x=self.attn_block(x)
            #print("postattnblock",x.shape)           

            #x = x * self.skip_scale

            x=self.attn_block(x)

            #x=einops.rearrange(x, 'b (c f) t -> b c f t', f=self.Fdim)
