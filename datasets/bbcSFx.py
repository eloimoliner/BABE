...
class AudioFolderDataset(torch.utils.data.IterableDataset):
    def __init__(self,
        ...
        filelist1=glob.glob(os.path.join(path,"*.wav"))
        filelist2=glob.glob(os.path.join(path,"*/*.wav"))
        #merge the two lists
        ...
        while True:
            ...
            while not good_sample: 
                ...
                if data.shape[-1]/samplerate > self.min_size:
                    ...
                    print(data.shape, "bad sample")
                if samplerate!=self.fs:
                    #just discard the sample, lazy solution
                    good_sample=False
            ...
            if not(self.stereo):
                ...
                #if the audio is mono, just repeat it
                if len(data.shape)==1:
